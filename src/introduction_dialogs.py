...
def free_version_dialog(user_name):
    show_message_dialog(
        PERSONAL_ASSISTANT_TITLE, f"{user_name}, Ви обрали безкоштовну версію."
    )
    show_message_dialog(
        PERSONAL_ASSISTANT_TITLE, "Ми від усієї команди бажаємо Вам вдалого дня!"
    )
    show_message_dialog(
        PERSONAL_ASSISTANT_TITLE, "Ha цьому функціонал безкоштовної версії скінчився."
    )
    ...
    if user_input == "1":
        show_message_dialog(
            ...
            f"{user_name}, тепер yci знають, хто не донатить. "
...
def chose_version_dialog(user_name):
    ...
    show_message_dialog(
        PERSONAL_ASSISTANT_TITLE,
        f"Вітаю, {user_name}! Наш бот абсолютно безкоштовний, "
    ...
    if user_input == "2":
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE, f"{user_name}, Ви зробили правильний вибір!"
        )
...
def donation_dialog():
    ...
    while donation_choice not in VALID_CHOICE:
        donation_choice = show_input_dialog(
            "Виберіть суму донату",
            "Будь ласка, оберіть суму донату:\n1. 10 грн\n2. 10 000$",
        )
    ...
    if donation_choice == "1":
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE, "Дякуємо за ваш донат y розмірі 10 грн!"
        )
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE, f"Вам надано доступ до {PERSONAL_ASSISTANT_TITLE}"
        )
    elif donation_choice == "2":
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE, "Дякуємо за ваш щедрий донат y розмірі 10 000$!"
        )
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE,
            "Ha жаль, ми виявили підозріло велику суму на вашому рахунку.",
        )
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE,
            "Ваші дані були відправлені Державній податковій службі України!",
        )
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE,
            "Але є i хороша новина - Ви тепер можете користуватися ботом!",
        )
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE, f"Вам надано доступ до {PERSONAL_ASSISTANT_TITLE}"
        )
...
def last_donation_dialog():
    ...
    if donation_response == "2":
        show_message_dialog(
            PERSONAL_ASSISTANT_TITLE,
            "Ваші дані успішно відправлені до найближчого ТЦК. Дякуємо, що обрали наш бот "
